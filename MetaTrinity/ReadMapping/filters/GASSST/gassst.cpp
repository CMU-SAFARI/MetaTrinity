...
/**
 * Structure d�finissant les arguments de la fonction appel�e par un thread
 */
typedef struct
{
	FILE * file;
...
/**
 * M�thode permettant d'afficher le temps d'ex�cution
 * \param temps le nombre total de secondes correspondant au temps d'ex�cution
 */
...
/**
 * M�thode associ�e � un thread
 * \param un pointeur vers une structure d'arguments thread_args
 * \return un pointeur void correspondant au nombre d'alignements trouv�s.
 */
...
/**
 * M�thode permettant de calculer les alignements pour une partition de la premi�re banque donn�e.
 * On va donc parcourir toutes les partitions de la seconde banque
 * \param B1 le pointeur de la premi�re banque de s�quences
 * \param B2 le pointeur de la seconde banque de s�quences
 * \param I1 le pointeur de l'index de la premi�re banque de s�quences
 * \param file le fichier de sortie, o� seront affich�s les alignements obtenus
 * \param dust_switch l'�tat d'activation du filtre Low complexity
 * \param percent_ident le pourcentage de ressemblances minimal entre les 2 s�quences pour qu'un alignement soit conserver
 * \param type_align le type des alignements, soit dans la s�quence normale ou dans la s�quence inverse compl�ment�e
 * \param num_gaps le nombre maximal de gaps autoris�s dans les alignements
 * \return un entier long corresspondant au nombre d'alignements trouv�s
 */
		...
		char** tabprec7, char** tabprec_gless, Stat ** St, Doublon ** Doub)
{
  //// Server * serveur = new Server(NB_DIFF_SEED, SIZE_PARTITION_THREAD); // achanger nquery, nquery par thread
	...
	/// Initialisation du verrou d'acc�s au fichier de sortie
	...
	while (B2->readBank(dust_switch)) //normalement un seul passage pour LQ
	{
		...
		/// R�cup�ration des r�sultats des threads
		for(num_thread=0; num_thread<NBTHREADS; ++num_thread)
		{
		...
		/// On r�initialise l'attribut tailleMaxSeq
		B2->tailleMaxSeq = 0;
...
/**
 * M�thode responsable du calcul des alignements selon certains param�tres
 * \param B1 le pointeur de la premi�re banque de s�quences
 * \param B2 le pointeur de la seconde banque de s�quences
 * \param I1 le pointeur de l'index de la premi�re banque de s�quences
 * \param file le fichier de sortie, o� seront affich�s les alignements obtenus
 * \param fileName le nom du fichier de sortie
 * \param rev_comp_state un bool�en qui indique si on utilise l'option reverse complement
 * \param dust_switch l'�tat d'activation du filtre Low complexity
 * \param percent_ident le pourcentage de ressemblances minimal entre les 2 s�quences pour qu'un alignement soit conserv�
 * \param num_gaps le nombre maximal de gaps autoris�s dans les alignements
 */
...
table_prec7 = (char **) malloc(dimbigt*sizeof(char *));
  ...
  while (B1->readBank(dust_switch))
    {
      /// Indexation de la premi�re banque
	...
	cout << endl << endl << "    [[ Normal + Reverse complement research ]]" << endl;
      ...
      /// On fait la recherche "reverse complement" si elle est activ�e 
...
*/
  ...
  /// On affiche les r�sultats trouv�s
  if(!rev_comp_state)
 ...
 */
int main(int argc,char * argv[]){ // Max Rumpf
	
  /// D�finition des banques de s�quences
  Bank * B1;
  Bank * B2;
  /// D�finition des index associ�s aux banques
  Index * I1;
  ...
  /// m�morisation du temps de d�part
  gettimeofday(&tv, 0);
...
/// Option nombre max d align par read
  ...
  /// Si la taille des partitions n'est pas d�finie, on la d�finit � 2% de la taille de la m�moire principale (RAM)
  ...
  /// Option reverse complement, on r�cup�re le nom du fichier de sortie
  ...
  /// D�finition de l'objet contenant statistiques des align trouv�s
  Stat * St[NBTHREADS]; //cela marche til ?

  /// R�cup�ration des noms des fichiers des banques
  ...
  /// Calcul du nombre de graines diff�rentes
  NB_DIFF_SEED = 1;
