 ...
 * \param size1 le nombre de bases � parcourir dans la premi�re s�quence
 * \param size2 le nombre de bases � parcourir dans la seconde s�quence
 ...
 * \return AL_VALID si l'alignement trouv� est correct, AL_ERR si on n'a pas retenu l'alignement
 */
...
int WN_alignment(Alignment* al, char* seq1, char* seq2, int size1, int size2, int num_gaps, int side, int num_seed, int max_mis)
{
  ...
  int dim1 = size1+1;
  int dim2 = size2+1;
  /// La matrice d'alignement, la premi�re s�quence constitue la premi�re dimension
  ...
  /// Variables utiles au contr�le de la validit� de l'alignement
  ...
  if(side == LEFT)
    {
      ...
      /// On se place au d�but des s�quences
      s1 = seq1 - size1;
      s2 = seq2 - size2;
    }
  else
    {
      ...
      /// Dans ce cas on inverse les s�quences
...
//     }
  ...
  /// On initialise la premi�re case de la matrice
  tab[0][0].score = 0;
  // On initialise les cases utiles de la premi�re colonne
  // Ppour premi�re colonne : gaps gratuits car differentes positions de depart possibles
  for(i=1; i <= min(num_gaps + num_gaps,size1); ++i) //bande de 2*numgaps de large
    {
      tab[i][0].score = 0;
      tab[i][0].path = GA2;
    }
  /// On initialise les cases utiles de la premi�re ligne
  ...
  /// On ne compl�te que les cases des diagonales respectant le nombre de gaps autoris�s
  for(i = 1; i < dim1; ++i)
    {
      ...
      for(j = binf; j <= bsup; ++j)
	{
	  ...
	  tab[i][j].score = -10000 ; 
	  tab[i][j].path = MES;
	  /// On v�rifie l'appariement des caract�res
	  ...
	  /// On teste la possibilit� d'ajouter un gap
  ...
  --i;
  --j;
  /// On parcourt le chemin repr�sentant le meilleur alignement dans la matrice
  /// On commence dans le coin en bas � droite de la matrice, correspondant aux derniers caract�res , au niveau de la graine 
  /// On va continuer jusqu'� ce que la s�quence la plus courte (la seconde) ait �t� compl�tement lue
  while(j>0)
    {
	...
	case GA1:
      ...
      /// Si on a d�j� utilis� trop de gaps, on arr�te la construction de l'alignement
...
//renvoit tableau de bits avec des 1 la o� lettres trouvees, reste a faire poopcount
...
//renvoit 1 si passe filtre, 0 si l'alignement est �limin�

// This is the base counting filter. It uses a slightly different formula (due to rounding error) than mrFAST
...
inline unsigned int get_code(unsigned int * tabcode,int tai_acc,int tai)
{
  ...
  int dec = (2*tai_acc)/8; //nombre d'octets � manger
  unsigned int * temp;
  tai_acc -= 4*dec;
  var += dec;
  temp =  (unsigned int *) var;
  int    decalage = 2*(4*sizeof(unsigned int) -tai - tai_acc);
  unsigned int masque = (1 << tai) -1 ;   //masque de 2*tai bit � 1 
...
inline unsigned int get_code(unsigned int *  vec,int tai_acc,int tai)
{
  ...
  int    decalage = 2*(4*sizeof(unsigned int) -tai - tai_acc);
  unsigned int masque = (1 << (2*tai)) -1 ;   //masque de 2*tai bit � 1 
  return ((temp>>decalage) &masque);
...
//renvoit 1 si passe filtre, 0 si l'alignement est �limin�
 ...
 int filtre_align( int left2, int right2, int max_mis,
		  ...
		  ,unsigned int fleft1,unsigned int fright1
		   ,unsigned int fleft2,unsigned int fright2,int num_gaps)
{
  /// On v�rifie le nombre de caract�res utilisables dans la premi�re s�quence
  ...
  while (tai_acc < 16 && tairest>0)
   ...
   masque = (1 << (2*tai)) -1 ; 
      ...
      for (err=1; err <= num_gaps; err++) //si err �lev�, depassement au premier coup, pas grave mais minore l'erreur
	{
...
{	 
  ...
  if(size > 0 && left2 > 0)
	...
	validity = WN_alignment(al, seq1, seq2, size, left2, num_gaps, LEFT, num_seed, max_err); 
      ...
      /// On v�rifie la validit� de l'alignement

      //  if(validity == AL_DOUBLON)  aldoublon++;
      if(validity != AL_VALID) return max_mis+1;
    }
  /// On ajoute la graine � l'appariement
			      ...
			      if(filtre_align(left2,right2,max_mis,tabprec,fleftB,frightB,fleft,fright,num_gaps)) //TWN(16)
				{
				  ...
				  if(! (Doub->getpos(absolute_pos))) // test position "fictive", cest a dire si pas de gaps // devenu obsolete il me semble
							 ...
							 fleftB,frightB,fleft,fright,num_gaps ))
					{
					  ...
					  /// Enregistrement des indexs de d�but et de fin des s�quences
					  ...
					  if (nb_mis<=max_mis )
					      ...
					      if( Doub->getpos( BK1->pos_seq[num_sequence] + al->getStart1()-1))//ultime verif non doublon, avec vrai position
						{
					      ...
					      /// On r�cup�re les index des commentaires des s�quences
						...
						{
						  St->incr_query();
						  /// Verrou pour prot�ger l'acc�s au fichier de sortie
						  pthread_rwlock_wrlock(&verrou_out);
