...
static inline void write_tags(kstring_t *s, const mm_reg1_t *r)
{
	...
	if (r->p) {
		mm_sprintf_lite(s, "\tNM:i:%d\tms:i:%d\tAS:i:%d\tnn:i:%d", r->p->dp_max2, r->p->dp_max, r->p->dp_score, r->p->n_ambi);
...
static void write_sam_cigar(kstring_t *s, int sam_flag, int in_tag, int qlen, const mm_reg1_t *r, int opt_flag, int Edits_total)
{
	if (r->p == 0) {
		mm_sprintf_lite(s, "*");
	} else {
		...
		if (in_tag) { // case 1
            ...
            if (r->p->dp_max2 == 0) {
                mm_sprintf_lite(s, "%d%s", qlen, "M");
            } else{
                mm_sprintf_lite(s, "%d%s", r->p->dp_max2, "I");   // r->p->dp_max2 = number of Edits from filters
                mm_sprintf_lite(s, "%d%s", (qlen - (r->p->dp_max2)), "M");
            }
            ...
            //originally used
            /*for (k = 0; k < r->p->dp_max2; ++k)
                mm_sprintf_lite(s, "%d%c", r->p->cigar[k]>>4, "MIDNSHP=XB"[r->p->cigar[k]&0xf]);*/

            //verbose
            /*printf("Edits CIGAR (case 2): %d\n", r->p->dp_max2);
            printf("read length (qlen): %d\n", qlen);
...
void mm_write_sam3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int opt_flag, int rep_len, int Edits_total)
{
	...
	if (r == 0) {
		...
		} else write_sam_cigar(s, flag, 0, t->l_seq, r, opt_flag, Edits_total);
	}
	...
	if (r) {
		...
		if (r->p && (opt_flag & (MM_F_OUT_CS|MM_F_OUT_MD)))
			write_cs_or_MD(km, s, mi, t, r, !(opt_flag&MM_F_OUT_CS_LONG), opt_flag&MM_F_OUT_MD, 1);
		if (cigar_in_tag)
			write_sam_cigar(s, flag, 1, t->l_seq, r, opt_flag, Edits_total);
	}
	if (rep_len >= 0) mm_sprintf_lite(s, "\trl:i:%d", rep_len);
...
void mm_write_sam2(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int opt_flag, int Edits_total)
{
	mm_write_sam3(s, mi, t, seg_idx, reg_idx, n_seg, n_regss, regss, km, opt_flag, -1, Edits_total);
}

void mm_write_sam(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, int n_regs, const mm_reg1_t *regs, int Edits_total)
{
	int i;
	for (i = 0; i < n_regs; ++i)
		if (r == &regs[i]) break;
	mm_write_sam2(s, mi, t, 0, i, 1, &n_regs, &regs, NULL, 0, Edits_total);
}
...
static int sam_print_rg_line(kstring_t *str, const char *s)
{
	char *p, *q, *r, *rg_line = 0;
	memset(mm_rg_id, 0, 256);
	if (s == 0) return 0;
...
int mm_print_sam_hdr(const mm_idx_t *idx, const char *rg, const char *ver, int argc, char *argv[])
{
	kstring_t str = {0,0,0};
	int ret = 0;
	if (idx) {
	...
	if (rg) ret = sam_print_rg_line(&str, rg);
	printf("@PG\tID:minimap2\tPN:minimap2");
	if (ver) printf("\tVN:%s", ver);
	if (argc > 1) {
		int i;
		printf("\tCL:minimap2");
		for (i = 1; i < argc; ++i)
...
static void print_cs_core(kstring_t *s, const uint8_t *tseq, const uint8_t *qseq, const mm_reg1_t *r, char *tmp, int no_iden, int print_tag)
{
	int i, q_off, t_off;
	if (print_tag) printf("\tcs:Z:");
	for (i = q_off = t_off = 0; i < (int)r->p->n_cigar; ++i) {
		int j, op = r->p->cigar[i]&0xf, len = r->p->cigar[i]>>4;
		assert((op >= 0 && op <= 3) || op == 7 || op == 8);
		if (op == 0 || op == 7 || op == 8) { // match
			...
			for (j = 0; j < len; ++j) {
				if (qseq[q_off + j] != tseq[t_off + j]) {
					...
					printf("*%c%c", "acgtn"[tseq[t_off + j]], "acgtn"[qseq[q_off + j]]);
				} else tmp[l_tmp++] = "ACGTN"[qseq[q_off + j]];
			}
			if (l_tmp > 0) {
				if (!no_iden) {
			...
			assert(len >= 2);
			printf("~%c%c%d%c%c", "acgtn"[tseq[t_off]], "acgtn"[tseq[t_off+1]],
				len, "acgtn"[tseq[t_off+len-2]], "acgtn"[tseq[t_off+len-1]]);
			t_off += len;
		}
	}
...
static void print_MD_core(kstring_t *s, const uint8_t *tseq, const uint8_t *qseq, const mm_reg1_t *r, char *tmp, int print_tag)
{
	int i, q_off, t_off, l_MD = 0;
	if (print_tag) printf("\tMD:Z:");
	for (i = q_off = t_off = 0; i < (int)r->p->n_cigar; ++i) {
		int j, op = r->p->cigar[i]&0xf, len = r->p->cigar[i]>>4;
		assert((op >= 0 && op <= 3) || op == 7 || op == 8);
		if (op == 0 || op == 7 || op == 8) { // match
...
static void print_cs_or_MD(void *km, kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, int no_iden, int is_MD, int print_tag)
{
	extern unsigned char seq_nt4_table[256];
	int i;
	uint8_t *qseq, *tseq;
	...
	if (is_MD) print_MD_core(s, tseq, qseq, r, tmp, print_tag);
	else print_cs_core(s, tseq, qseq, r, tmp, no_iden, print_tag);
	kfree(km, qseq); kfree(km, tseq); kfree(km, tmp);
}

static inline void print_tags(kstring_t *s, const mm_reg1_t *r)
{
	int type;
	if (r->id == r->parent) type = r->inv? 'I' : 'P';
	else type = r->inv? 'i' : 'S';
	if (r->p) {
		printf("\tNM:i:%d\tms:i:%d\tAS:i:%d\tnn:i:%d", r->blen - r->mlen + r->p->n_ambi, r->p->dp_max, r->p->dp_score, r->p->n_ambi);
		if (r->p->trans_strand == 1 || r->p->trans_strand == 2)
			printf("\tts:A:%c", "?+-?"[r->p->trans_strand]);
	}
	printf("\ttp:A:%c\tcm:i:%d\ts1:i:%d", type, r->cnt, r->score);
	if (r->parent == r->id) printf("\ts2:i:%d", r->subsc);
	if (r->p) {
		char buf[16];
		double div;
		div = 1.0 - mm_event_identity(r);
	...
	if (r->split) printf("\tzd:i:%d", r->split);
}

void mm_print_paf3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, void *km, int opt_flag, int rep_len)
{
	s->l = 0;
	if (r == 0) {
		printf("%s\t%d\t0\t0\t*\t*\t0\t0\t0\t0\t0\t0", t->name, t->l_seq);
		if (rep_len >= 0) printf("\trl:i:%d", rep_len);
		return;
	}
	printf("%s\t%d\t%d\t%d\t%c\t", t->name, t->l_seq, r->qs, r->qe, "+-"[r->rev]);
	if (mi->seq[r->rid].name) printf("%s", mi->seq[r->rid].name);
	else printf("%d", r->rid);
	printf("\t%d\t%d\t%d", mi->seq[r->rid].len, r->rs, r->re);
	printf("\t%d\t%d", r->mlen, r->blen);
	printf("\t%d", r->mapq);
	print_tags(s, r);
	if (rep_len >= 0) printf("\trl:i:%d", rep_len);
	if (r->p && (opt_flag & MM_F_OUT_CG)) {
		...
		printf("\tcg:Z:");
		for (k = 0; k < r->p->n_cigar; ++k)
			printf("%d%c", r->p->cigar[k]>>4, "MIDNSHP=XB"[r->p->cigar[k]&0xf]);
	}
	if (r->p && (opt_flag & (MM_F_OUT_CS|MM_F_OUT_MD)))
		print_cs_or_MD(km, s, mi, t, r, !(opt_flag&MM_F_OUT_CS_LONG), opt_flag&MM_F_OUT_MD, 1);
	if ((opt_flag & MM_F_COPY_COMMENT) && t->comment)
		printf("\t%s", t->comment);
}

void mm_print_paf(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, void *km, int opt_flag)
{
	mm_print_paf3(s, mi, t, r, km, opt_flag, -1);
}

static void sam_print_sq(kstring_t *s, char *seq, int l, int rev, int comp)
{
	extern unsigned char seq_comp_table[256];
	if (rev) {
		int i;
...
static void print_sam_cigar(kstring_t *s, int sam_flag, int in_tag, int qlen, const mm_reg1_t *r, int opt_flag)
{
	if (r->p == 0) {
		printf("*");
	} else {
		...
		if (in_tag) {
			int clip_char = (sam_flag&0x800) && !(opt_flag&MM_F_SOFTCLIP)? 5 : 4;
			printf("\tCG:B:I");
			if (clip_len[0]) printf(",%u", clip_len[0]<<4|clip_char);
			for (k = 0; k < r->p->n_cigar; ++k)
				printf(",%u", r->p->cigar[k]);
			if (clip_len[1]) printf(",%u", clip_len[1]<<4|clip_char);
		} else {
			int clip_char = (sam_flag&0x800) && !(opt_flag&MM_F_SOFTCLIP)? 'H' : 'S';
			assert(clip_len[0] < qlen && clip_len[1] < qlen);
			if (clip_len[0]) printf("%d%c", clip_len[0], clip_char);
			for (k = 0; k < r->p->n_cigar; ++k)
				printf("%d%c", r->p->cigar[k]>>4, "MIDNSHP=XB"[r->p->cigar[k]&0xf]);
			if (clip_len[1]) printf("%d%c", clip_len[1], clip_char);
		}
	}
}

void mm_print_sam3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int opt_flag, int rep_len)
{
	const int max_bam_cigar_op = 65535;
	int flag, n_regs = n_regss[seg_idx], cigar_in_tag = 0;
	int this_rid = -1, this_pos = -1, this_rev = 0;
	...
	// print coordinate, MAPQ and CIGAR
	if (r == 0) {
		if (r_prev) {
			this_rid = r_prev->rid, this_pos = r_prev->rs;
			printf("\t%s\t%d\t0\t*", mi->seq[this_rid].name, this_pos+1);
		} else printf("\t*\t0\t0\t*");
	} else {
		this_rid = r->rid, this_pos = r->rs, this_rev = r->rev;
		printf("\t%s\t%d\t%d\t", mi->seq[r->rid].name, r->rs+1, r->mapq);
		if ((opt_flag & MM_F_LONG_CIGAR) && r->p && r->p->n_cigar > max_bam_cigar_op - 2) {
			int n_cigar = r->p->n_cigar;
			if (r->qs != 0) ++n_cigar;
			if (r->qe != t->l_seq) ++n_cigar;
		...
		if (cigar_in_tag) {
			...
			printf("%dS%dN", slen, r->re - r->rs);
		} else print_sam_cigar(s, flag, 0, t->l_seq, r, opt_flag);
	}

	// print mate positions
	if (n_seg > 1) {
		int tlen = 0;
		if (this_rid >= 0 && r_next) {
			if (this_rid == r_next->rid) {
				...
				printf("\t=\t");
			} else printf("\t%s\t", mi->seq[r_next->rid].name);
			printf("%d\t", r_next->rs + 1);
		} else if (r_next) { // && this_rid < 0
			printf("\t%s\t%d\t", mi->seq[r_next->rid].name, r_next->rs + 1);
		} else if (this_rid >= 0) { // && r_next == NULL
			printf("\t=\t%d\t", this_pos + 1); // next segment will take r's coordinate
		} else printf("\t*\t0\t"); // neither has coordinates
		if (tlen > 0) ++tlen;
		else if (tlen < 0) --tlen;
		printf("%d\t", tlen);
	} else printf("\t*\t0\t0\t");
	...
	// print SEQ and QUAL
	if (r == 0) {
		sam_print_sq(s, t->seq, t->l_seq, 0, 0);
		printf("\t");
		if (t->qual) sam_print_sq(s, t->qual, t->l_seq, 0, 0);
		else printf("*");
	} else {
		if ((flag & 0x900) == 0 || (opt_flag & MM_F_SOFTCLIP)) {
			sam_print_sq(s, t->seq, t->l_seq, r->rev, r->rev);
			printf("\t");
			if (t->qual) sam_print_sq(s, t->qual, t->l_seq, r->rev, 0);
			else printf("*");
		} else if (flag & 0x100) {
			printf("*\t*");
		} else {
			sam_print_sq(s, t->seq + r->qs, r->qe - r->qs, r->rev, r->rev);
			printf("\t");
			if (t->qual) sam_print_sq(s, t->qual + r->qs, r->qe - r->qs, r->rev, 0);
			else printf("*");
		}
	}

	// print tags
	if (mm_rg_id[0]) printf("\tRG:Z:%s", mm_rg_id);
	if (n_seg > 2) printf("\tFI:i:%d", seg_idx);
	if (r) {
		print_tags(s, r);
		if (r->parent == r->id && r->p && n_regs > 1 && regs && r >= regs && r - regs < n_regs) { // supplementary aln may exist
			int i, n_sa = 0; // n_sa: number of SA fields
			...
			if (n_sa > 0) {
				...
				for (i = 0; i < n_regs; ++i) {
					...
					clip5 = q->rev? t->l_seq - q->qe : q->qs;
					clip3 = q->rev? q->qs : t->l_seq - q->qe;
					printf("%s,%d,%c,", mi->seq[q->rid].name, q->rs+1, "+-"[q->rev]);
					if (clip5) printf("%dS", clip5);
					if (l_M) printf("%dM", l_M);
					if (l_I) printf("%dI", l_I);
					if (l_D) printf("%dD", l_D);
					if (clip3) printf("%dS", clip3);
					printf(",%d,%d;", q->mapq, q->blen - q->mlen + q->p->n_ambi);
				}
			}
		}
		if (r->p && (opt_flag & (MM_F_OUT_CS|MM_F_OUT_MD)))
			print_cs_or_MD(km, s, mi, t, r, !(opt_flag&MM_F_OUT_CS_LONG), opt_flag&MM_F_OUT_MD, 1);
		if (cigar_in_tag)
			print_sam_cigar(s, flag, 1, t->l_seq, r, opt_flag);
	}
	if (rep_len >= 0) printf("\trl:i:%d", rep_len);
...
void mm_print_sam2(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int opt_flag)
{
	mm_print_sam3(s, mi, t, seg_idx, reg_idx, n_seg, n_regss, regss, km, opt_flag, -1);
}

void mm_print_sam(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, const mm_reg1_t *r, int n_regs, const mm_reg1_t *regs)
{
	int i;
	for (i = 0; i < n_regs; ++i)
		if (r == &regs[i]) break;
	mm_print_sam2(s, mi, t, 0, i, 1, &n_regs, &regs, NULL, 0);
}
